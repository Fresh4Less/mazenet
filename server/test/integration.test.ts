...
import { from, fromEvent, forkJoin, of, Observable, timer } from 'rxjs';
import { delay, map, mergeMap, first } from 'rxjs/operators';
...
class Client {
    ...
    public emitTransaction(method: string, route: string, body: any): Observable<Client> {
        ...
        const obs: Observable<Response> = fromEvent(this.client, route);
        this.client.emit(route, {
            body,
            method,
        });
        return forkJoin(of(request), obs.pipe(first())).pipe(
    ...
    public connectAndEnter(): Observable<Client> {
        return this.connectSocket().pipe(
            mergeMap((client) => {
...
beforeEach(() => {
    ...
    return server.start().pipe(
        map(() => {
            ...
            baseUrl = `${protocol}://127.0.0.1:${(server.httpServer.address() as AddressInfo).port}/mazenet`;
            if(postgres) {
                ...
                return server.postgresPool!.query(
                    ...
                    return forkJoin(
                        result.rows.map((row) => from(server.postgresPool!.query(`TRUNCATE ${row.table_name} CASCADE`)))
                    ).toPromise();
                });
            }
        })
...
describe('users', () => {
    test('POST /connect', () => {
        ...
        return client.connectSocket().pipe(
            map(() => {
...
describe('single client', () => {
    describe('rooms', () => {
        test('POST /enter', () => {
            ...
            return client.connectAndEnter().pipe(
                map(() => {
                    const res = client.res;
                    expect(res.status).toBe(200);
                    // room
